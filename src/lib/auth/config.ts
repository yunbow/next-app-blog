...
import { getPlanFromPriceId, isSubscriptionActive } from "@/lib/stripe";
...
export const authConfig: NextAuthConfig = {
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger }) {
      if (user) {
        ...
        if (user.email) {
          const dbUser = await prisma.user.findUnique({
            where: { email: user.email },
            select: { username: true, isAdmin: true, isSuspended: true, stripePriceId: true, subscriptionStatus: true },
          });
          if (dbUser) {
            token.username = dbUser.username;
            token.isAdmin = dbUser.isAdmin;
            token.isSuspended = dbUser.isSuspended;
            const plan = getPlanFromPriceId(dbUser.stripePriceId);
            token.plan = isSubscriptionActive(dbUser.subscriptionStatus) ? plan : "free";
          }
        }
      }
      ...
      // 実装前に発行されたJWTはplanを持たないため、次回リフレッシュ時に補完する
      if (token.plan === undefined && token.id) {
        const dbUser = await prisma.user.findUnique({
          where: { id: token.id as string },
    ...
    async session({ session, token }) {
      ...
      if (token && session.user) {
        ...
        session.user.plan = token.plan as "free" | "basic" | "premium" | undefined;
      }
      return session;
    },
  },
