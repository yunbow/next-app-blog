...
export default async function ArticleDetailPage({ params, searchParams }: Props) {
  const [{ slug }, { commentPage: commentPageParam }] = await Promise.all([params, searchParams]);
  const commentPage = Math.max(1, Number(commentPageParam) || 1);
  ...
  const [reactionStats, userReactions, { comments, totalPages: commentTotalPages }, bookmarkData, collections] = await Promise.all([
    getReactionStats(article.id),
    userId ? getUserReactions(article.id, userId) : Promise.resolve([]),
    getArticleComments(article.id, commentPage),
  ...
  return (
    <div className="container max-w-3xl pb-8">
      ...
      {article.thumbnail && getImageUrl(article.thumbnail) && (
