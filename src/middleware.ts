...
export default auth(async function middleware(request) {
  // CSP nonce must be set on request headers so server components can read it
  // via headers() — middleware response headers are not accessible from server components
  ...
  if (["POST", "PUT", "DELETE", "PATCH"].includes(request.method)) {
    ...
    const isValidOrigin = origin && isSameHost(origin, host ?? "");
    const isValidReferer = referer && isSameHost(referer, host ?? "");
