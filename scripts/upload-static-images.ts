...
async function main() {
  ...
  const files = await collectImageFiles(STATIC_DIR);

  if (files.length === 0) {
    console.log("No images found in public/static/");
    return;
  }
  ...
  for (const filePath of files) {
    const key = relative(join(process.cwd(), "public"), filePath).replace(/\\/g, "/");
